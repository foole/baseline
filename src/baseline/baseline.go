...
func (tr *TestResult) RunTest(test string, artifactDir string) {
	...
	if fs.IsDir() {
		panic("Got directory ", test, " instead of file")
	}
	if fs.Mode() & 0555 != 0555 {
		panic("File ", test, " is not executable")
	}
	...
	if out.Len() != 0 {
		e := ioutil.WriteFile(artifactDir/testName.stdout, out, 0644)
		if e != nil {
			panic("Unable to write file", artifactDir/testName.stdout)
		}
	}
	if err.Len() != 0 {
		e := ioutil.WriteFile(artifactDir/testName.stderr, err, 0644)
		if e != nil {
			panic("Unable to write file", artifactDir/testName.stderr)
		}
	}
}

func CreateBaseline(test string, artifactDir string) string {
	result = RunTest(test, artifactDir)
